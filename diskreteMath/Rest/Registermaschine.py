            ...
            if command_nr < 0:
                ...
                if ret is None:
                    ...
                    if len(ret) != len(params) +  1:
                        raise TypeError(f"Subprogramm {actual} benötigt {len(params)} Parameter, aber es sind {len(ret) - 1} angegeben.")
...
class Context:
    ...
    def __setitem__(self, key, value):
        if not key.startswith('$') or len(key) < 2:
            raise ValueError(f"Illegaler Parametername '{key}'; Parameternamen beginnen mit einem '$' end besitzen Länge >= 2")
    ...
    def __check_arguments(cls, code, args):
        ...
        for i in range(len(args)):
            if code[i] == 'r': 
                ...
                if len(args[i]) != 1:
                    raise ValueError(f"Das {i+1}. Argument von {cmd} sollte ein einzelnes Symbol sein, ist aber {args[i]}")
